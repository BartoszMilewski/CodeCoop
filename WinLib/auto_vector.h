...
public:
	...
	std::unique_ptr<T> extract(size_t i);
	// stack access
...
// return resource, null the entry
template <class T>
std::unique_ptr<T> auto_vector<T>::extract(size_t i)
{
	Assert (i < size ());
	std::unique_ptr<T> result(_arr[i]);
	_arr[i] = 0;
	return result;
}
